...
class ContainerService:
    ...
    async def get_all_containers(self) -> Result[List[Container], Exception]:
        try:
            containers = []
            async for doc in self.db_service.db["containers"].find({}):
                container = Container.from_dict(doc)
                containers.append(container)
            return containers
        except Exception as e:
            Logger.error(f"Error in get_all_containers: {e}")
            return Result.Err(e)
    ...
    async def get_container_metrics(
        ...
        # metrics = await self.api_service.containers.get_container_metrics(
        #     user, container
        # )

        # Logger.debug(f"Metrics for container {container.id}: {metrics}")
        return {}
    ...
    async def delete_container(
        ...
        api_result = await self.api_service.containers.delete_container(
            user_id, container_id
        )

        if api_result.is_err():
            return Err(api_result.unwrap())
